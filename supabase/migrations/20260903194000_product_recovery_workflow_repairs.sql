-- Repair the deployed workflow functions after live integration testing exposed
-- an ambiguous PL/pgSQL actor variable.  The definitions below also make a
-- one-off slot expire when it starts: after that instant there is no longer a
-- defensible reason to contact the lead.
...
-- Persisted deferral keeps a booked/closed task from starving the rest of
-- Today's queue while retaining the underlying result state.
alter table public.sf_leads
  add column if not exists next_review_at timestamptz;

create index if not exists sf_leads_org_next_review_idx
  on public.sf_leads (organization_id, next_review_at)
  where next_review_at is not null;

create or replace function public.sf_approve_recovery_message(
  p_organization_id uuid,
  p_message_id uuid,
  p_body text
)
...
begin
  ...
  if char_length(btrim(coalesce(p_body, ''))) < 10 then
    raise exception using errcode = '22023', message = 'MESSAGE_BODY_REQUIRED';
  end if;
  ...
  if message_row.status not in ('draft', 'copied') then
    raise exception using errcode = '23514', message = 'MESSAGE_NOT_APPROVABLE';
  end if;
...
create or replace function public.sf_reconcile_stale_work(
  p_organization_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  with retired as (
    update public.sf_recommendations recommendation
     ...
     where recommendation.organization_id = p_organization_id
       ...
       and (
         (recommendation.expires_at is not null and recommendation.expires_at <= now())
         or (change.type = 'slot' and (change.starts_at is null or change.starts_at <= now()))
         or (
           change.type = 'availability'
           and (change.ends_at is null or change.ends_at <= now())
         )
       )
  ...
  with reset as (
    update public.sf_leads lead
     ...
     where lead.organization_id = p_organization_id
       and lead.status = 'approval'
       and not lead.dnc
       and not lead.medical_escalation
       and not lead.needs_fix
       and lead.stopped_reason_code <> 'unknown'
       and not exists (
         select 1
           from public.sf_messages message
           join public.sf_recommendations recommendation
             on recommendation.id = message.recommendation_id
            and recommendation.organization_id = message.organization_id
          where message.organization_id = lead.organization_id
  ...
  if retired_count > 0 or reset_count > 0 then
    insert into public.sf_activity (organization_id, action, details, actor_id)
    values (
      p_organization_id,
      'stale_work_reconciled',
      jsonb_build_object('retired', retired_count, 'reset', reset_count),
      actor_user_id
    );
  end if;

  return jsonb_build_object('retired', retired_count, 'reset', reset_count);
end;
$$;
...
create or replace function public.sf_record_follow_up_request(
  p_organization_id uuid,
  p_message_id uuid,
...
begin
  ...
  if char_length(btrim(coalesce(p_response_text, ''))) < 2 then
    raise exception using errcode = '22023', message = 'RESPONSE_TEXT_REQUIRED';
  end if;

  clinic_timezone := coalesce(
    (select clinic.timezone
       from public.sf_clinics clinic
      where clinic.organization_id = p_organization_id),
    'Asia/Jerusalem'
  );
  local_today := (now() at time zone clinic_timezone)::date;
  if p_requested_date is null
     or p_requested_date < local_today
     or p_requested_date > local_today + 365 then
    raise exception using errcode = '22023', message = 'INVALID_REQUESTED_DATE';
  end if;

  transition := public.sf_record_recovery_response(
    p_organization_id,
    p_message_id,
    'not_now',
    p_response_text
  ...
  select * into lead_row
    from public.sf_leads
   where id = (transition -> 'lead' ->> 'id')::uuid
     and organization_id = p_organization_id
   for update;
  ...
  if not lead_row.dnc and not lead_row.medical_escalation then
    follow_up_at := ((p_requested_date::timestamp + time '09:00') at time zone clinic_timezone);
    update public.sf_leads
       set status = 'watching',
           stopped_reason_code = 'requested_date',
           stopped_reason_text = 'לחזור בתאריך ' || to_char(p_requested_date, 'DD.MM.YYYY'),
           requested_contact_after = p_requested_date,
           next_review_at = follow_up_at,
  ...
  values (
    p_organization_id,
    lead_row.id,
    'recovery_follow_up_requested',
    jsonb_build_object(
      'requested_date', p_requested_date,
      'scheduled', not lead_row.dnc and not lead_row.medical_escalation
    ),
    actor_user_id
  );

  return jsonb_set(transition, '{lead}', to_jsonb(lead_row), true);
end;
$$;
...
revoke all on function public.sf_approve_recovery_message(uuid, uuid, text)
  from public, anon, authenticated;
revoke all on function public.sf_reconcile_stale_work(uuid)
  from public, anon, authenticated;
revoke all on function public.sf_defer_recovery_progress(uuid, uuid, timestamptz)
  from public, anon, authenticated;
revoke all on function public.sf_record_follow_up_request(uuid, uuid, text, date)
  from public, anon, authenticated;
revoke all on function public.sf_correct_recovered_revenue(uuid, uuid, bigint, text)
  from public, anon, authenticated;

grant execute on function public.sf_approve_recovery_message(uuid, uuid, text)
  to authenticated;
grant execute on function public.sf_reconcile_stale_work(uuid)
  to authenticated;
grant execute on function public.sf_defer_recovery_progress(uuid, uuid, timestamptz)
  to authenticated;
grant execute on function public.sf_record_follow_up_request(uuid, uuid, text, date)
  to authenticated;
