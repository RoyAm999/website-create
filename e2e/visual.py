"""Real browser responsive, scroll and guest-entry acceptance."""
...
async def main():
 async with async_playwright() as p:
  ...
  for name,w,h,reduce in [('desktop',1440,1000,False),('mobile',390,844,False),('small',360,640,False),('reduced',1440,1000,True)]:
   ...
   if report[-1]['leadsOverflow']:
    report[-1]['overflowElements']=await page.evaluate("Array.from(document.querySelectorAll('main *')).map(e=>({tag:e.tagName,cls:e.className,left:e.getBoundingClientRect().left,right:e.getBoundingClientRect().right,width:e.getBoundingClientRect().width})).filter(e=>e.left < -1 || e.right > innerWidth+1).slice(0,30)")
   ...
   await ctx.close()
  await browser.close()
  result={'devices':report,'errors':errors}
  (OUT/'visual-report.json').write_text(json.dumps(result,ensure_ascii=False,indent=2))
